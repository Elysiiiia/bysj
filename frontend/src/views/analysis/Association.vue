  ...
  <AppLayout :title="title">
    <div class="assoc-layout">
      <div class="assoc-main-chart">
        ...
        <div v-if="type === 'association1'" style="flex:1;padding:16px;overflow:hidden;display:flex;flex-direction:column;gap:16px">
          <div ref="chartA" style="flex:1;min-height:0"></div>
        </div>

        <div v-else-if="type === 'association2'" style="flex:1;display:grid;grid-template-rows:1fr 1fr;overflow:hidden">
      ...
      <div class="assoc-sidebar">
        ...
        <div v-else-if="type === 'association2'" class="card" style="flex:1;overflow:hidden">
          ...
          <div class="card-body" style="padding:8px;height:calc(100% - 44px)">
            <div ref="chartC" style="width:100%;height:100%"></div>
          </div>
        </div>
          ...
          <div class="card-body" style="padding:10px;overflow-y:auto;height:calc(100% - 44px)">
            ...
            <div v-for="(rule, index) in rules" :key="index" style="margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #f1f5f9">
              <div style="font-size:12px;font-weight:600;margin-bottom:4px">
                <span style="color:#2563eb">{{ rule.antecedent }}</span>
                <span style="color:#94a3b8;margin:0 5px">⇄</span>
                <span style="color:#16a34a">{{ rule.consequent }}</span>
              </div>
          ...
          <div class="card-body" style="padding:12px;font-size:12px;color:#64748b;line-height:1.8">
            <template v-if="type === 'association1'">
              <div style="display:flex;gap:8px;margin-bottom:6px;align-items:center">
                <span style="width:14px;height:14px;background:#16a34a;border-radius:3px"></span>
                <span>正面评价：情感分≥0.75</span>
              </div>
              <div style="display:flex;gap:8px;margin-bottom:6px;align-items:center">
                <span style="width:14px;height:14px;background:#f59e0b;border-radius:3px"></span>
                <span>中性评价：情感分 0.40~0.75</span>
              </div>
              <div style="display:flex;gap:8px;align-items:center">
                <span style="width:14px;height:14px;background:#ef4444;border-radius:3px"></span>
                <span>负面评价：情感分＜0.40</span>
              </div>
            </template>
            <template v-else-if="type === 'association2'">
              <div v-for="(item, index) in specRank" :key="item.spec" style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #f1f5f9">
                <span>{{ index + 1 }}. {{ item.spec }}</span>
                <span :style="{ color:rankColor(item.avgSentimentPct), fontWeight:700 }">{{ item.avgSentimentPct }}%</span>
              </div>
            </template>
            <template v-else>
...
function rankColor(v) {
  return v >= 70 ? '#16a34a' : v >= 50 ? '#f59e0b' : '#ef4444'
}
...
function renderAssociation1() {
  echarts.init(chartA.value).setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    legend: { top: 4, itemWidth: 12, textStyle: { fontSize: 12 } },
    grid: { left: '12%', right: '4%', top: '12%', bottom: '8%' },
    xAxis: { type: 'value' },
    yAxis: { type: 'category', data: data.value.brands || [] },
    series: (data.value.series || []).map((item) => ({
      name: item.name,
      type: 'bar',
      stack: 'total',
      data: item.data,
      itemStyle: { color: ({ 正面: '#16a34a', 中性: '#f59e0b', 负面: '#ef4444' })[item.name] }
    }))
  })
}

function renderAssociation2() {
  const specs = [...(data.value.specData || [])].sort((a, b) => b.avgSentimentPct - a.avgSentimentPct)
  echarts.init(chartA.value).setOption({
    tooltip: { trigger: 'axis' },
    grid: { left: '14%', right: '6%', top: '4%', bottom: '18%' },
    xAxis: { type: 'category', data: specs.map((i) => i.spec), axisLabel: { fontSize: 10, rotate: 30 } },
    yAxis: { type: 'value', min: 0, max: 100, axisLabel: { formatter: (v) => `${v}%` } },
    series: [{ type: 'bar', data: specs.map((i) => i.avgSentimentPct), label: { show: true, position: 'top', formatter: '{c}%' } }]
  })
  echarts.init(chartB.value).setOption({
    tooltip: { trigger: 'axis' },
    grid: { left: '12%', right: '6%', top: '8%', bottom: '18%' },
    xAxis: { type: 'category', data: (data.value.priceSentiment || []).map((i) => i.range), axisLabel: { rotate: 15 } },
    yAxis: { type: 'value', min: 0, max: 1, axisLabel: { formatter: (v) => `${Math.round(v * 100)}%` } },
    series: [{ type: 'line', smooth: true, data: (data.value.priceSentiment || []).map((i) => i.avg), areaStyle: { opacity: 0.2 } }]
  })
  echarts.init(chartC.value).setOption({
    tooltip: { formatter: (params) => `评分：${params.value[0]}<br>情感：${(params.value[1] * 100).toFixed(0)}%` },
    grid: { left: '12%', right: '6%', top: '12%', bottom: '12%' },
    xAxis: { type: 'value', name: '评分', min: 1, max: 5 },
    yAxis: { type: 'value', name: '情感分', min: 0, max: 1, axisLabel: { formatter: (v) => `${Math.round(v * 100)}%` } },
    series: [{ type: 'scatter', symbolSize: 6, data: (data.value.scatter || []).map((i) => [i.x, i.y]) }]
  })
}

function renderAssociation3() {
  const nodes = (data.value.nodes || []).map((node) => ({
    ...node,
    itemStyle: { color: '#2563eb' },
    label: { show: true, fontSize: 10 }
  }))
  const heatData = data.value.heatData || data.value.heat_data || []
  echarts.init(chartA.value).setOption({
    tooltip: { formatter: (params) => params.dataType === 'edge' ? `${params.data.source}<br>↔ ${params.data.target}<br>共现：${params.data.value} 次` : params.name },
    series: [{
      type: 'graph',
      layout: 'force',
      roam: true,
  ...
  echarts.init(chartB.value).setOption({
    ...
    xAxis: { type: 'category', data: data.value.brands || [], splitArea: { show: true } },
    yAxis: { type: 'category', data: data.value.brands || [], splitArea: { show: true } },
    visualMap: { min: 0, max: Math.max(...heatData.map((i) => i[2] || 0), 1), orient: 'horizontal', left: 'center', bottom: 0, inRange: { color: ['#eff6ff', '#2563eb'] } },
    series: [{ type: 'heatmap', data: heatData }]
  })
}

function render() {
  if (type.value === 'association1') renderAssociation1()
  else if (type.value === 'association2') renderAssociation2()
  else renderAssociation3()
}
