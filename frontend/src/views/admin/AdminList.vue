      ...
      <div class="card-body">
        <div class="table-toolbar">
          ...
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <button class="btn btn-success" @click="handleAddClick">+ 新增{{ typeName }}</button>
          </div>
        </div>
            ...
            <tbody>
              <tr v-for="row in rows" :key="row.id">
                <td v-for="col in columns" :key="col.key">
                  ...
                  <span v-else-if="col.kind === 'yesno'" class="badge" :class="isTruthy(row[col.key]) ? 'badge-success' : 'badge-secondary'">{{ isTruthy(row[col.key]) ? '是' : '否' }}</span>
...
const columns = computed(() => ({
  phones: [
    { key: 'brand', label: '品牌', kind: 'brand' },
    { key: 'title', label: '商品标题', truncate: 2 },
    { key: 'currentPrice', label: '当前价格', kind: 'price' },
    { key: 'originalPrice', label: '原价', kind: 'money-muted' },
    { key: 'sales', label: '销量' },
    { key: 'shopName', label: '店铺名称', truncate: 1 },
    { key: 'govSubsidy', label: '政府补贴', kind: 'yesno' },
    { key: 'selfOperated', label: '自营', kind: 'yesno' },
    { key: 'avgRating', label: '平均评分', kind: 'rating' },
    { key: 'sentimentScore', label: '情感分值', kind: 'sentiment' }
  ],
  comments: [
    { key: 'brand', label: '品牌', kind: 'brand' },
    { key: 'nickname', label: '用户昵称' },
    { key: 'rating', label: '评分', kind: 'stars' },
    { key: 'spec', label: '商品规格' },
    { key: 'content', label: '评论内容', truncate: 2 },
    { key: 'sentimentLabel', label: '情感标签', kind: 'sentimentLabel' },
    { key: 'sentimentScore', label: '情感分值', kind: 'sentiment' },
    { key: 'commentDate', label: '评论日期' }
  ],
  users: [
    { key: 'username', label: '用户名' },
    { key: 'role', label: '角色', kind: 'role' },
...
async function load() {
  const res = await api.get(endpoint.value, {
    params: { page: page.value, pageSize: pageSize.value, q: q.value }
  })
...
function handleAddClick() {
  if (type.value === 'comments') {
    router.push('/admin/comments/add')
    return
  }
  openForm()
}
...
function isTruthy(value) {
  return value === true || value === '是' || value === 'Y' || value === 1
}
